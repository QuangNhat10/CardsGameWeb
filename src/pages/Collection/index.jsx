...
const initialCards = Array.from({ length: 20 }, (_, i) => {
	...
	return {
		...
		abilities: [
			`Ability 1 for Card ${i + 1}`,
			`Ability 2 for Card ${i + 1}`,
		],
	...
	return (
		<div className="page">
			...
			<div className="container">
				<h2 style={{ textAlign: 'center', margin: '24px 0' }}>Cards Collection</h2>
				<div className="collection-controls" style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 8 }}>
					<div className="collection-sort-btn" onClick={() => setShowDropdown(v => !v)}>
						...
						>
							<div style={{ marginBottom: 12 }}>
								<label style={{ display: 'block', marginBottom: 8 }}>
									<input type="checkbox" checked={sortAZ} onChange={() => setSortAZ(s => !s)} /> Sắp xếp tên A-Z
								</label>
								<label style={{ display: 'block', marginBottom: 4 }}>Lọc theo loại:</label>
								{CARD_TYPES.map(type => (
							...
							<div style={{ textAlign: 'right', marginTop: 12 }}>
								<button className="modal-close-btn" onClick={() => setShowDropdown(false)}>Close</button>
							</div>
						</div>
					</div>
				)}
				<div className="collection-cards">
					{filteredCards.map(card => (
						<div key={card.id} onClick={() => setSelectedCard(card)} className="collection-card">
							<MagicCard
								name={card.name}
			...
			{selectedCard && (
				<div className="collection-modal" onClick={() => setSelectedCard(null)}>
					<div className="collection-modal-content" onClick={e => e.stopPropagation()}>
						...
						<div className="collection-modal-info">
							<strong>Loại:</strong> {selectedCard.rarity}<br />
							<strong>Cấp độ:</strong> Lv.{selectedCard.level}
						</div>
						<button className="collection-modal-close" onClick={() => setSelectedCard(null)}>Close</button>
					</div>
				</div>
			)}
			<Footer />
